...
RUN adduser --system --uid 1001 cpe-coder
...
COPY --from=builder --chown=cpe-coder:nodejs /builder/node_modules /runner/node_modules
COPY --from=builder --chown=cpe-coder:nodejs /builder/dist /runner/dist
COPY --from=builder --chown=cpe-coder:nodejs /builder/package.json /runner/package.json
